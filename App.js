      ...
      async function requestExternalWritePermission() {
        try {
          const granted = await PermissionsAndroid.request(
            ...
            {
              title: 'Emotions Decoded External Storage Write Permission',
              message:
                'Emotions Decoded needs access to Storage data in your SD Card ',
            }
          );
          if (granted === PermissionsAndroid.RESULTS.GRANTED) {
      ...
      async function requestExternalReadPermission() {
        try {
          const granted = await PermissionsAndroid.request(
            ...
            {
              title: 'Emotions Decoded Read Storage Read Permission',
              message: 'Emotions Decoded needs access to your SD Card ',
            }
          );
          if (granted === PermissionsAndroid.RESULTS.GRANTED) {
  ...
  async takePicture(event) {
    ...
    try {
      ...
      Object.keys(result[0].faceAttributes.emotion).forEach(key => {
        if (
          this.state &&
          result[0].faceAttributes.emotion[key] > this.state.maxEmotionNumber
        ) {
          this.setState({maxEmotion: key});
          this.setState({
            maxEmotionNumber: result[0].faceAttributes.emotion[key],
          });
        }
      });
    } catch (ex) {
    ...
    Tts.getInitStatus().then(() => {
      Tts.speak(
        'The most likely emotion they are feeling is:' + this.state.maxEmotion,
      );
    });
    Tts.stop();
  }
